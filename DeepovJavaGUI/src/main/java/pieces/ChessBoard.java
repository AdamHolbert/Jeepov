		...
		for(int a = 0; a < 8; a++){
			for(int b = 0; b < 8; b++){
				...
				if(i < 32){
					...
					int x = g.getBoardPieces().get(i).getPosition().getX();
					int y = g.getBoardPieces().get(i).getPosition().getY();
					String simpleName = g.getBoardPieces().get(i).getClass().getSimpleName();
					Color color = g.getBoardPieces().get(i).getColor();
